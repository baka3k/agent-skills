...
variables:
  ...
  loop:
    - name: $CURRENT_FILE
      source: State.current_file (set by file_worklist loop)
      required: false
    - name: $CURRENT_INDEX
...
## 🔄 Variable Passing Mechanism (Cách truyền biến giữa các agents)

> **Quy tắc:** Mọi biến `$VARIABLE` trong agent files được resolve tại runtime bởi LangGraph state.
> Không có biến nào là "magic" — tất cả đều đến từ 1 trong 3 nguồn sau:

### 1. CLI Arguments (Đầu vào từ người dùng → State)

| Biến | Nguồn | Gán khi | Agent nào dùng |
|------|-------|---------|---------------|
| `$SOURCE_FOLDER` | `--source-folder` CLI | Khởi tạo state | **Tất cả** |
| `$MODULE_NAME` | `--module` CLI | Khởi tạo state | Orchestrator, Pre-porting, Plan-gen |
| `$OUTPUT_DIR` | `--output` CLI (mặc định: `porting-output/`) | Khởi tạo state | **Tất cả** |
| `$DRY_RUN` | `--dry-run` CLI | Khởi tạo state | Orchestrator, Pre-porting |

```
main.py --source-folder /path/to/cpp --module SampleModule --output ./output
    │
    ▼
PortingState = {
    source_folder: "/path/to/cpp",    → $SOURCE_FOLDER
    module:       "SampleModule",           → $MODULE_NAME
    output_dir:   "./output",         → $OUTPUT_DIR
    dry_run:      false               → $DRY_RUN
}
```

### 2. MCP Graph Data (Dữ liệu từ Graph → State → Biến loop)

| Biến | Nguồn | Được set bởi | Agent nào dùng |
|------|-------|-------------|---------------|
| `$FILE_WORKLIST` | `plan_dependency_order()` + scan | Plan-generator (Phase 5) | File-structure |
| `$CURRENT_FILE` | Lặp qua `$FILE_WORKLIST` | File-structure router | File-structure, Function |
| `$CURRENT_INDEX` | Index trong worklist | File-structure router | File-structure |
| `$FUNCTION_LIST` | `get_symbol()` cho file hiện tại | File-structure (Step 1) | Function |
| `$FUNCTION_INDEX` | Index trong function list | Function router | Function |

```
[Plan-generator]
    │
    ├─ scan + plan_dependency_order
    │
    ▼
State.file_worklist = ["SampleModule/SampleModuleConfig.cpp", "SampleModule/CVtgmData.cpp", ...]
State.file_worklist[0] → $CURRENT_FILE = "SampleModule/SampleModuleConfig.cpp"
State.current_file_index = 0          → $CURRENT_INDEX
    │
    ▼
[File-structure] dùng MCP Graph scan file hiện tại
    │
    ▼
State.function_list = [{name:"Load", ...}, {name:"Save", ...}]
State.function_list[0] → next function to port
State.current_function_index = 0      → $FUNCTION_INDEX
```

### 3. Self-Managed (State tracking, CI/CD loop counters)

| Biến | Mục đích | Được quản lý bởi |
|------|----------|-----------------|
| `$ARGUMENTS` | Raw user input string (fallback) | LangGraph entry node |
| `$FIRST_MODULE` | Module đầu tiên trong plan | Plan-generator output |

### Cách resolve trong LangGraph implementation

```python
# Trong mỗi agent node, biến $VARIABLE được resolve từ state:
def resolve_prompt(template: str, state: PortingState) -> str:
    """Resolve $VARIABLE placeholders từ state"""
    mapping = {
        "$SOURCE_FOLDER": state["source_folder"],
        "$MODULE_NAME": state["module"],
        "$OUTPUT_DIR": state["output_dir"],
        "$CURRENT_FILE": state.get("current_file", ""),
        "$CURRENT_INDEX": str(state.get("current_file_index", 0)),
        "$WORKLIST": str(state.get("file_worklist", [])),
        "$FUNCTION_LIST": str(state.get("function_list", [])),
        "$FUNCTION_INDEX": str(state.get("current_function_index", 0)),
        "$FIRST_MODULE": first_module(state),
    }
...
### Agent Variable Declaration (mỗi agent PHẢI khai báo)

Mỗi agent file cần có section khai báo biến đầu vào để LangGraph biết cách resolve:

```yaml
# Ví dụ trong frontmatter hoặc đầu agent body:
variables:
  input:
    - name: $SOURCE_FOLDER
      source: CLI --source-folder
...
7. **Gán global sequential ID**: Mỗi function nhận 1 ID duy nhất (`F001`..`F{N}`) **KHÔNG phân theo wave/class**
8. **Consolidated full listing**: Gộp tất cả vào 1 bảng duy nhất đánh STT 1→N

### 1.5.3 Output

```
porting-output/porting-plan/
├── porting-plan.md            # ✅ Consolidated full listing (tất cả functions, global STT)
│                                + grouped by wave + leaf nodes + circular deps
├── porting-plan.json          # Machine-readable (có conversion_list flat)
├── leaf-nodes.md              # 🌿 Parallel-executable functions (F-ID list)
└── circular-deps.md           # ⚠️ Circular dependencies + resolution
```

### 1.5.4 Sử dụng Plan

- **Phase 2 (File Structure)**: Đọc `porting-plan.json` → port file theo wave order, dùng F-ID
- **Phase 3 (Function)**: Đọc function order trong plan → port từng function theo STT/ID
- **Parallel execution**: Các function trong cùng wave + là leaf → port song song
- **Tracking**: Dùng F-ID để track tiến độ (vd: "hoàn thành F001-F023", "đang làm F024")
